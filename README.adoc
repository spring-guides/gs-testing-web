...
Then create a test case with the `@RunWith` and `@SpringBootTest` annotations:
...
NOTE: A nice feature of the Spring Test support is that the application context is cached in between tests, so if you have multiple methods in a test case, or multiple test cases with the same configuration, they only incur the cost of sytarting the application once. You can control the cache using the {DirtiesContext}[`@DirtiesContext`] annotation.
...
[source,java]
----
include::complete/src/test/java/hello/HttpRequestTest.java[tag=test]
----
...
[source,java]
----
include::complete/src/test/java/hello/ApplicationTest.java[tag=test]
----
...
[source,java]
----
include::complete/src/test/java/hello/WebMockTest.java[tag=test]
----
