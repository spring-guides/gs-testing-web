...
NOTE: The above example does not specify `GET` vs. `PUT`, `POST`, and so forth, because `@RequestMapping` maps all HTTP operations by default. Use `@GetMapping` or `@RequestMapping(method=GET)` to narrow this mapping.
